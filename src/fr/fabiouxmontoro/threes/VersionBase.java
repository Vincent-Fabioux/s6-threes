...
public class VersionBase extends JPanel implements KeyListener, MouseListener{
	...
	private final static int TILES_NB = TILES_NB_L * TILES_NB_L;	// Nombre de tuiles total
	private final static int TILES_SIZE = 124;						// Taille des tuiles
	private final static int TILES_POLICE_SIZE = TILES_SIZE / 3;	// Taille de la police des tuiles
	private final static int TILES_GAP = 3;							// Ecart entre chaque tuile
	private final static int TILES_INIT_MIN = 4;					// Nombre de tuiles minimum au démarrage
	private final static int TILES_INIT_MAX = 6;					// Nombre de tuiles maximum au démarrage
	
	private final static int WINDOW_SIZE_X = TILES_SIZE * TILES_NB_L + TILES_GAP * (TILES_NB_L + 1);	// Largeur de la fenêtre
	private final static int WINDOW_SIZE_Y = TILES_SIZE * TILES_NB_L + TILES_GAP * (TILES_NB_L + 1);	// Hauteur de la fenêtre
	...
	private void initGame(int window_Size_X,int window_Size_Y)
	{
		...
		// Tuiles du jeu
		int i;
	    Font police = new Font("Arial", Font.BOLD, TILES_POLICE_SIZE);	//On definit la police
		tiles = new JLabel[TILES_NB]; // On initialise les JLabel des tuiles
		tilesValue = new int[TILES_NB]; //On initilaise le tableau contenant les valeurs des tuiles
		for(i = 0; i < TILES_NB; i++)
		{
			...
			tiles[i].setHorizontalAlignment(javax.swing.SwingConstants.CENTER); // On place l'ecriture au centre du JLabel
			tiles[i].setBorder(BorderFactory.createLineBorder(Color.BLACK)); // On definit les contours de chaque JLabel
		}
	}
	...
	private void renderTiles()
	{
		...
		{
			// Si la tuile a comme valeur 0, on affiche un caractere vide sur le JLabel
			if(tilesValue[i] == 0){
				tiles[i].setText("");
